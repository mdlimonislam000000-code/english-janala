...
const displayLevelWords = (words) => {
    ...
    if(words.length==0){
        wordContainer.innerHTML=`
            <div class="col-span-full text-center space-y-5 font-bangla">
            <img class="mx-auto" src="./assets/alert-error.png" alt="">
            <p class="text-[1.3rem] font-medium text-gray-400">এই Lesson এ এখনো কোন Vocabulary যুক্ত করা হয়নি।</p>
            <h1 class="font-semibold text-[2.5rem]">নেক্সট Lesson এ যান</h1>
        </div>
        `;
    }
    words.forEach(word => {
...
const displayLesson = (lessons) => {
    ...
    for(let lesson of lessons) {
        ...
        btnDiv.innerHTML = `
        <button id="lesson-btn-${lesson.level_no}" onClick="loadLevelWord(${lesson.level_no})" class=" lesson-btn btn btn-outline btn-primary mr-3"><i class="fa-solid fa-book-open"></i>Lesson ${lesson.level_no} </button>
        `;
